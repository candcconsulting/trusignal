...
import { EmphasizeElements, IModelApp, NotifyMessageDetails, OutputMessagePriority, StandardViewId } from "@itwin/core-frontend";
...
export async function checkBS1192 () : Promise<void>{
/* 
 load the BS1192.json rules
 and validate against them
*/
    IModelApp.notifications.outputMessage(new NotifyMessageDetails(OutputMessagePriority.Info, " starting BS1192 checks ..."));
    ...
    for (const ruleset of Object.keys(rules.rules))
    {
        ...
        for await (const aResult of aResults) {
            ...
            if (aResult.param1) {
                ...
                console.log (aRule.checksql + ' returned an invalid resultset');
                aFound = true;
            }
            if (!aFound) {
                ...
                for await (const invalidResult of invalidResults) {
                    console.log("ID : " + invalidResult.id + ", UserLabel : " + invalidResult.userlabel+ "Class : "+ invalidResult.ecclass);
