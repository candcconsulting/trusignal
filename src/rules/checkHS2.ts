...
export async function checkHS2 () : Promise<void>{
/* 
    ...
    for (const ruleset of Object.keys(rules.rules))
    {
        ...
        for await (const aResult of aResults) {
            ...
            if (!aFound) {
                ...
                let instanceQuery = aRule.elementsql.replace('<param1>', aResult.param1);
                instanceQuery = aRule.elementsql.replace('<param2>', aResult.param2);
