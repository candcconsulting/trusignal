...
import { DecorateContext, Decorator, GraphicBuilder, GraphicType, IModelApp, Marker, RenderGraphic } from "@itwin/core-frontend";
import { Arc3d, CurveChainWithDistanceIndex, GeometryQuery, IndexedPolyface, IndexedPolyfaceVisitor, LineSegment3d, LineString3d, Loop, Path, Point3d } from "@itwin/core-geometry";
...
/** A decorator that simplifies the process of drawing common geometry types.  The decorator
 * must be added to a viewport using ViewManager.addDecorator.  Once added, call methods setColor,
 * setFill, etc. to establish the desired appearance.  Then call addLine, addArc, etc. to add
 * geometry primitives.  The graphics will be generated and displayed as needed.
 */
export class GeometryDecorator implements Decorator {
  ...
  private _add<T>(array: T[], val: T) {
    array.push (val);
    IModelApp.viewManager.invalidateCachedDecorationsAllViews(this);
  }

  public addMarker(marker: Marker) {
    this._add(this.markers, marker);
  }
  ...
  public addLine(line: LineSegment3d) {
    ...
    this._add(this.shapes, styledGeometry);
  }
  ...
  public addPoint(point: Point3d) {
    ...
    this._add(this.points, styledPoint);
  }
  ...
  public decorate(context: DecorateContext): void {
    ...
    const graphics = this.createGraphics(context);
    if (graphics)
      context.addDecoration(GraphicType.Scene, graphics);
